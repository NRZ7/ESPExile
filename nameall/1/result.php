...
class Core {
	...
	public function lastkill()
	{
		$img = '<h3>Last Kills<align="center"/></h3>';
        echo $img;
        $account_name = $_POST['account_name'];
		$select = mysqli_query($this->sql, 'SELECT name, died_at, death_log, killer_name  FROM player_history WHERE killer_name = "'.$account_name.'" ORDER BY `died_at` DESC LIMIT 5;');
		echo('<table width="1300px" id="stats" class="tablesorter" border="0" cellspacing="0" align="center">');
		echo('<thead><tr>');
	    echo('	<th>Died at</th>');
		echo('	<th>Killer</th>');
	...
	public function longkill()
	{
		$img = '<h3>Longest Kills<align="center"/></h3>';
        echo $img;
        $account_name = $_POST['account_name'];
		$select = mysqli_query($this->sql, 'SELECT name, died_at, death_log, killer_name, distance  FROM player_history WHERE killer_name = "'.$account_name.'" ORDER BY `distance` DESC LIMIT 3;');
		echo('<table width="1300px" id="stats" class="tablesorter" border="0" cellspacing="0" align="center">');
		echo('<thead><tr>');
	    echo('	<th>Died at</th>');
		echo('	<th>Distance</th>');
